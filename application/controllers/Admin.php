...
class Admin extends CI_Controller {
	...
	public function generate_callback() {
		if($this->input->post()){
			...
			$data=array(
				...
				'leadid'=> trim("CSG-".sprintf("%'.011d",$lead_ids).PHP_EOL),
				'user_id'=>$user_name,
				'due_date'=>$due_date,
	...
	public function email_report() {
		...
		if($reportType != 'dailyCallback'){
			...
			$tmparry[] = 'info@countrysidegroup.co.in';
	...
	function bulk_upload_callback() {
		if(isset($_POST["submit"])) {
			...
			if (move_uploaded_file($_FILES["upload_xl"]["tmp_name"], $target)){
				...
				//$leadIdKey = "CSG - ".sprintf("%'.011d",$lead_ids).PHP_EOL;
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_POSTFIELDS => "xml=<?xml version='1.0'?><query><user_name>countryside99</user_name><pswd>ind123</pswd><start_date>2020-02-19 00:00:00</start_date><end_date>2020-02-20 23:59:59</end_date></query>",
